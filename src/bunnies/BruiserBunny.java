...
public class BruiserBunny extends TeamRobot {
	private static final double BULLET_POWER = 2.1;
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		BunnyWave wave = new BunnyWave(this);
		wave.gunLocation = new Point2D.Double(getX(), getY());
		BunnyWave.targetLocation = GFTUtils.project(wave.gunLocation,
...
class BunnyWave extends Condition {
	...
	BunnyWave(AdvancedRobot _robot) {
		this.robot = _robot;
	}
...
class GFTMovement {
	...
	private static final double DEFAULT_EVASION = 1.4;
	...
	GFTMovement(AdvancedRobot _robot) {
		...
		fieldRectangle = new Rectangle2D.Double(WALL_MARGIN, WALL_MARGIN,
				robot.getBattleFieldWidth() - WALL_MARGIN * 2, robot.getBattleFieldHeight()
						- WALL_MARGIN * 2);
	}
